...
def create_service_category(category_data: ServiceCategoryCreate, user_id: str = None):
    if user_id is None:
    ...
    try:
        ...
        category_data_dict['user_id'] = user_id
...
async def delete_service_category(id: str):
    service_category = storage.find_by(ServiceCategory, **{"id": id})
    if service_category is None:
        raise HTTPException(status_code=404, detail="Service category not found")
...
async def create_service_price_package(package_data: PricePackageCreate, service_id: str):
...
async def update_service_price_package(id: str, package_data: PricePackageUpdate):
    ...
    try:
        package_data_dict = package_data.dict(exclude_unset=True)
        for key, value in package_data_dict.items():
            setattr(price_package, key, value)
